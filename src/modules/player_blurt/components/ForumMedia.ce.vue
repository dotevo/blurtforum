...
// Global listener for <forum-media> tags generated by Parser -- this is a
// fallback safety net only. The component's own internal click handlers
// (handlePlay/handleQueue, all using @click.stop) already fully handle
// clicks inside the shadow DOM on their own; this listener should rarely
// if ever fire for a well-formed card, since those internal handlers stop
// propagation before a click can reach here.
if (typeof document !== 'undefined') {
  document.addEventListener('click', (e: MouseEvent) => {
    ...
    if (forumMedia) {
      ...
      // Guard against acting on missing/garbled data -- e.g. a stray click
      // that reaches here with attributes that didn't parse as expected
      // should be a no-op, not a call with `undefined`/malformed values.
      const looksSane = (v: string | undefined) => !!v && !/[<>"'\s]/.test(v);
      if (!looksSane(d.type) || !looksSane(d.id) || !looksSane(d.host)) return;
      e.preventDefault();
    ...
    <template v-else-if="mode === 'card'">
      ...
      >
        ...
        <div class="wt-actions">
          <button type="button" class="btn btn-primary btn-sm" @click.stop="handlePlay" :disabled="trackData.pending">
            <i :class="isPlaying ? 'fa-solid fa-pause' : 'fa-solid fa-play'"></i>
            <span>{{ isPlaying ? 'Pause' : t('playTorrent') }}</span>
          </button>
          <button type="button" class="btn btn-ghost btn-sm" @click.stop="handleQueue" :disabled="trackData.pending">
            <i class="fa-solid fa-plus"></i> <span>Queue</span>
          </button>
          <button type="button" class="btn btn-ghost btn-sm" @click.stop="copyMagnetLink(trackData.sources[0].id)">
      ...
      <div 
        ...
        @click.stop="() => { if (!hideButtons) handlePlay(); }"
      >
        <div v-if="!hideButtons" class="media-placeholder-overlay">
          <div class="media-placeholder-actions">
            <button type="button" class="btn btn-primary" @click.stop="handlePlay" :disabled="trackData.pending">
            ...
            <button type="button" class="btn btn-ghost" @click.stop="handleQueue" :disabled="trackData.pending">
